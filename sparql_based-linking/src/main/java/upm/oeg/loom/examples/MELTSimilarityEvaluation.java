...
public class MELTSimilarityEvaluation {
  ...
  public static void main(String[] args) {
    ...
    // --------------------------------------------------------
    ExecutionResultSet executionResultSetAnatomy =
        Executor.loadFromAnatomyResultsFolder(pathToAnatomyResultFiles);
    MatcherSimilarityMetric metric = new MatcherSimilarityMetric();
    MatcherSimilarity similarity =
        metric.get(
            executionResultSetAnatomy, TrackRepository.Anatomy.Default.getTestCases().get(0));
    PrintWriter writer = new PrintWriter(System.out);
    MatcherSimilarityLatexHeatMapWriter.write(similarity, writer);
    System.out.println("\nMedian: " + similarity.getMedianSimiarity());
    System.out.println(
        "Median (no self reference): "
            + similarity.getMedianSimilariyWithoutSelfSimilarity()
            + "\n\n\n");

    // --------------------------------------------------------
    // MAD Calculation Anatomy
    // --------------------------------------------------------
    MatcherSimilarityLatexPlotWriter.write(similarity, writer);

    // --------------------------------------------------------
    // Heat Map Evaluation Conference
    // --------------------------------------------------------
    ExecutionResultSet executionResultSetConference =
        Executor.loadFromConferenceResultsFolder(pathToConferenceResultFiles);
    metric = new MatcherSimilarityMetric();
    similarity =
        metric.get(
            MatcherSimilarityMetric.CalculationMode.MICRO,
            executionResultSetConference,
